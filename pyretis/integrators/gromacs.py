...
class GromacsExt(ExternalScript):
    ...
    def execute_until(self, system, order_function, settings,
        ...
        order = self.calculate_order(order_function,
                                     system)
        tpr_file = None
        cpt_file = None
    ...
    def propagate(self, path, system, order_function, interfaces, reverse=False):
        ...
        exe_dir = '/mnt/documents/src/pyretis/examples/molecular_dynamics/external_integrator/gromacs/test'

        success = False
        left, _, right = interfaces

        # Save as a single snapshot file
        system.particles.set_pos((initial_conf, None, None))
        ext_time = self.time_step * self.subcycles
        ...
        for i in range(path.maxlen):
            ...
            order = self.calculate_order(order_function, system)
            print(order, system.particles.pos_file)
            print(path)
            print('***')
            os.remove(conf_abs)
        ...
        print(system.particles.pos_file)
    ...
    def dump_frame(self, system):
        ...
        """
        pos = system.particles.pos_file
        idx = pos[1]
        if idx is None:
            return pos[0]
        else:
            basepath = os.path.dirname(pos[0])
            out_file = os.path.join(basepath, 'conf.g96')
            self.get_trr_frame(pos[0], self.input_file['tpr'],
                               idx, out_file)
            return out_file
