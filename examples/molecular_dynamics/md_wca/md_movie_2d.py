...
size = np.array([[0.0, 3.4], [0.0, 3.4]]) # in lj-units
box = Box(size)
...
for i,pos in enumerate(lattice):
    ljsystem.add_particle(name='A', pos=pos, mass=1.0, ptype='A')
# mutate two particles to be of type B
ljsystem.particles.ptype[7] = 'B'
ljsystem.particles.ptype[8] = 'B'
...
linebond, = ax.plot(None, None, lw=2, ls='-', color='magenta', alpha=1)
...
rpot = np.linspace(0.1, 5, 1000)
vpot = []
pos1 = np.array([xi for xi in ljsystem.particles.pos[7]])
pos2 = np.array([xi for xi in ljsystem.particles.pos[8]])
size = np.array([[0.0, 10.0], [0.0, 10.0]]) # in lj-units
fakebox = Box(size)
for ri in rpot:
    ljsystem.particles.pos[7] = np.array([0.0, 0.0])
    ljsystem.particles.pos[8] = np.array([ri, 0.0])
    v = dwca.potential(ljsystem.particles, fakebox)
    vpot.append(v)
ljsystem.particles.pos[7] = np.array([xi for xi in pos1])
ljsystem.particles.pos[8] = np.array([xi for xi in pos2])
print(ljsystem.particles.pos)
linedwpot, = ax3.plot(rpot, vpot, lw=2, ls='-', color='black')
ax3.set_ylim(0,dwca.height+1)
min_max = dwca.min_max()
ax3.set_xlim(min_max[0]-0.2, min_max[1]+0.2)
orderscatter = ax3.scatter(None, None, alpha=0.5, color='green', marker='s', s=150)
...
def update(i):
    ...
    if simulation_eq.is_finished():
        ...
        # reaction coordinate:
        delta = box.pbc_dist_coordinate(ljsystem.particles.pos[7] -
                                        ljsystem.particles.pos[8])
        ...
        pos2 = pos[8,:] + delta
        linebond.set_data([pos[8,0]*SIGMA, pos2[0]*SIGMA],[pos[8,1]*SIGMA, pos2[1]*SIGMA])
        patches.append(linebond)
        ...
        linepot.set_data(time, (np.array(v_pot)-v_pot[0])*ECONV)
        patches.append(linepot)
        linekin.set_data(time, (np.array(e_kin)-e_kin[0])*ECONV)
        patches.append(linekin)
        linetot.set_data(time, (np.array(e_tot)-e_tot[0])*ECONV)
        patches.append(linetot)
