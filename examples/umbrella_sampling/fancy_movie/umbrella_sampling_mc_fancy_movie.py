...
V = np.array([forcefield.evaluate_potential(pos=xi) for xi in xv]) 
...
fig.subplots_adjust(left=0.1, bottom=0.2, right=0.90, top=0.90, wspace=0.3, hspace=0.2)
...
for i, (traj, ener) in enumerate(zip(trajectory, energy)):
    ...
    ax_pot.set_xlabel('$x$')
    ax_pot.plot(xv, V, color='blue', lw=3, alpha=0.5)
    ax_pot.axvspan(xmin=umbrellas[i][0], xmax=umbrellas[i][1], color='blue', alpha=0.1)
    ax_pot.set_xlim(lim)
    ax_pot.set_ylim(potlim)
    scatter = ax_pot.scatter(None, None, s=150, c='green', alpha=0.6) # most recent
    ...
    for j in range(npoint):
        ...
        scatter_last.append(ax_pot.scatter(None, None, s=150, c='green', alpha=alphaj))
    ...
    rects = ax_hist.bar(bin_mid-0.5*db, hist, db, color=colors[i], alpha=0.5)

    ax_all_hist.set_xlim(lim)
    rects2 = ax_all_hist.bar(bin_mid-0.5*db, hist, db, color=colors[i], alpha=0.5)
    ...
    for j, (posj, enerj) in enumerate(zip(pos, ener)):
        ...
        p = np.array([posj, enerj])
        scatter.set_offsets(p) # update plot
        ...
        hist2, bins2, bin_mid2 = histogram(pos_so_far, bins=bins, limits=histlim, density=True)
        ax_hist.set_ylim(0.0, hist.max()*1.05)
        ymax = max([histi[0].max() for histi in all_normed_histograms] + [hist2.max()])
...
histograms_s, scale_factors, hist_avg = match_all_histograms(all_histograms, umbrellas)
...
for i, (histo, scale) in enumerate(zip(all_histograms, scale_factors)):
    ...
    if i==0:
        ...
        ax_hist_log.bar(bin_mid-0.5*db, hist, db, color=colors[i], alpha=0.5, log=True)
        ax_hist.set_ylim(0.0, hist.max()*1.05)
        ax_hist_log.set_ylim(logmin/10, hist.max()*10)
    else:
        s = np.linspace(1, scale, 12) # do scaling in 12 steps
        rects = ax_hist.bar(bin_mid-0.5*db, hist, db, color=colors[i], alpha=0.5)
        rects2 = ax_hist_log.bar(bin_mid-0.5*db, hist, db, color=colors[i], alpha=0.5, log=True)
