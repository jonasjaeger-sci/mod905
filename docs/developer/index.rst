...
and contributors are requested to familiarize themselves with this style and *use it*.
...
However, note that the use of ``logging.NullHandler()`` requires **Python 2.7** or later.
Also note that we do not add any particular handlers here - it is up to the *user* to
